    ...
    mainSteps = [
      {
        ...
        inputs = {
          runCommand = [
            ...
            "  ",
            "  # Remove uid and id, and set custom title to avoid conflicts",
            "  jq --arg name \"$${DASHBOARD_NAME}\" 'del(.uid, .id) | .title = $name' \"$${TEMP_DASHBOARD}\" > \"$${TEMP_DASHBOARD}.clean\"",
            "  mv \"$${TEMP_DASHBOARD}.clean\" \"$${TEMP_DASHBOARD}\"",
            "  ",
            ...
            "  fi",
            "  ",
            "  # Create import payload with folder assignment (overwrite: false to prevent replacing existing)",
            "  jq -n \\",
            "    --slurpfile dashboard \"$${TEMP_DASHBOARD}\" \\",
            "    --argjson inputs \"$${DS_INPUTS}\" \\",
            "    --argjson folderId \"$${FOLDER_ID:-0}\" \\",
            "    '{dashboard: $dashboard[0], overwrite: false, inputs: $inputs, folderId: $folderId}' > \"$${TEMP_IMPORT}\"",
            "  ",
            "  # Import to Grafana",
            ...
            "    echo \"[SUCCESS] Dashboard $${DASHBOARD_NAME} imported successfully (UID: $${IMPORTED_UID})\"",
            "  else",
            "    echo \"[ERROR] Failed to import $${DASHBOARD_NAME}\"",
            "    echo \"[ERROR] Response: $IMPORT_RESPONSE\"",
            "    echo \"[DEBUG] Dashboard JSON (first 500 chars):\"",
            "    head -c 500 \"$${TEMP_DASHBOARD}\"",
            "    echo \"\"",
            "  fi",
            "  ",
            ...
            "fi",
            "",
            "# Import dashboards built for kube-prometheus-stack",
            "echo \"[Grafana] Starting dashboard imports to EKS Monitoring folder\"",
            "echo \"[Grafana] Current dashboards in folder:\"",
            "curl -s \"$${GRAFANA_ENDPOINT}/api/search?type=dash-db&folderIds=$${FOLDER_ID}\" \\",
            "  -H \"Authorization: Bearer $${API_KEY}\" | jq -r '.[] | \"  - \\(.title)\"'",
            "echo \"\"",
            "",
            "# dotdc/grafana-dashboards-kubernetes - Modern dashboards for kube-prometheus-stack",
            "echo \"[Grafana] === Kubernetes Core Dashboards ===\"",
            "import_dashboard \"15757\" \"Kubernetes / Views / Global\"",
            "import_dashboard \"15758\" \"Kubernetes / Views / Namespaces\"",
            "import_dashboard \"15759\" \"Kubernetes / Views / Nodes\"",
            "import_dashboard \"15760\" \"Kubernetes / Views / Pods\"",
            "",
            "# System Components",
            "echo \"[Grafana] === Kubernetes System Components ===\"",
            "import_dashboard \"15761\" \"Kubernetes / System / API Server\"",
            "import_dashboard \"15762\" \"Kubernetes / System / CoreDNS\"",
            "",
            "# Prometheus Monitoring",
            "echo \"[Grafana] === Prometheus Monitoring ===\"",
            "import_dashboard \"19105\" \"Prometheus\"",
            "",
            "# Karpenter Autoscaling (2024 dashboards)",
            "echo \"[Grafana] === Karpenter Autoscaling ===\"",
            "import_dashboard \"22171\" \"Kubernetes / Autoscaling / Karpenter / Overview\"",
            "import_dashboard \"22172\" \"Kubernetes / Autoscaling / Karpenter / Activity\"",
            "import_dashboard \"22173\" \"Kubernetes / Autoscaling / Karpenter / Performance\"",
            "",
            "# Node Metrics",
            "echo \"[Grafana] === Node Exporter ===\"",
            "import_dashboard \"1860\" \"Node Exporter Full\"",
            "",
            "# Classic Overview Dashboard",
            "echo \"[Grafana] === Cluster Overview ===\"",
            "import_dashboard \"6417\" \"Kubernetes Cluster (Prometheus)\"",
            "",
            "echo \"[Grafana] Dashboard import complete\"",
            "",
            "# Optional: Delete old/duplicate dashboards",
            "echo \"[Grafana] Cleaning up old dashboards\"",
            "delete_dashboard() {",
            "  local DASHBOARD_UID=\"$1\"",
            "  local DASHBOARD_NAME=\"$2\"",
            "  ",
            "  DELETE_RESPONSE=$(curl -s -X DELETE \\",
            "    \"$${GRAFANA_ENDPOINT}/api/dashboards/uid/$${DASHBOARD_UID}\" \\",
            "    -H \"Authorization: Bearer $${API_KEY}\")",
            "  ",
            "  if echo \"$DELETE_RESPONSE\" | jq -e '.message' | grep -q \"deleted\"; then",
            "    echo \"[INFO] Deleted dashboard: $${DASHBOARD_NAME}\"",
            "  else",
            "    echo \"[INFO] Could not delete $${DASHBOARD_NAME} (may not exist)\"",
            "  fi",
            "}",
            "",
            "# To delete specific dashboards, uncomment and add their UIDs:",
            "# You can find UIDs by going to the dashboard and looking at the URL",
            "# Example: /d/abc123xyz/dashboard-name -> UID is 'abc123xyz'",
            "#",
            "# delete_dashboard \"old-dashboard-uid\" \"Old Dashboard Name\"",
            "",
            "echo \"[Grafana] Configuration complete!\"",
            "echo \"[Grafana] Access URL: $${GRAFANA_ENDPOINT}\"",
            "echo \"\"",
            "echo \"To delete unwanted dashboards:\"",
            "echo \"1. Go to the dashboard in Grafana\"",
            "echo \"2. Look at the URL: /d/<UID>/dashboard-name\"",
            "echo \"3. Add: delete_dashboard '<UID>' 'Dashboard Name' to the cleanup section\"",
          ]
        }
      }
    ]
