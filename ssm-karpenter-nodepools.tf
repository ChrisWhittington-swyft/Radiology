    ...
    mainSteps = [
      {
        ...
        inputs = {
          ...
          runCommand = [
            ...
            "  set +e",
            "  kubectl get ec2nodeclass default-ec2nodeclass -n karpenter >/dev/null 2>&1",
            "  EC2NC_EXISTS=$?",
            "  kubectl get nodepool default-nodepool -n karpenter >/dev/null 2>&1",
            "  NP_EXISTS=$?",
            "  set -e",
            "  ",
            "  if [ $EC2NC_EXISTS -eq 0 ] && [ $NP_EXISTS -eq 0 ]; then",
            "    echo \"[Karpenter] Resources already exist - skipping apply\"",
            "    echo \"[Karpenter] To update resources, manually delete them first:\"",
            "    echo \"  kubectl delete ec2nodeclass default-ec2nodeclass\"",
            "    echo \"  kubectl delete nodepool default-nodepool\"",
            "    exit 0",
            "  else",
            "    echo \"[Karpenter] ERROR: API discovery failed and resources don't exist\"",
            "    echo \"[Karpenter] This may indicate Karpenter controller is not running properly\"",
